...
export default function TempleDetail() {
  ...
  // typed helper: accepts either a multilingual object (Record<string,string>) or a plain string
  const display = (field?: Record<string, string> | string): string => {
    if (!field) return "-";
    if (typeof field === "string") return field;
    // field is a record of language -> text
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      {temple.mainDeity && (
        <p className="text-gray-700 mb-2">
          🕉️ <strong>Main Deity:</strong> {display(temple.mainDeity)}
        </p>
      )}
      ...
      <div className="border rounded-lg p-4 my-4 bg-white">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <p>
            <strong>Deity Description:</strong> {display(temple.deityDescription)}
          </p>
          <p>
            <strong>Significance:</strong> {display(temple.significance)}
          </p>
          <p>
            <strong>History:</strong> {display(temple.history)}
          </p>
          <p>
            <strong>Architecture:</strong> {display(temple.architecture)}
          </p>
          <p>
            <strong>Builder / Trust:</strong> {display(temple.builderOrTrust)}
          </p>
          <p>
            <strong>Consecration Date:</strong> {temple.consecrationDate || "-"}
          </p>
        </div>
      </div>
      ...
      <div className="border rounded-lg p-4 my-4 bg-white">
        ...
        <p>
          <strong>Darshan Timing:</strong> {display(temple.darshanTiming)}
        </p>
        <p>
          <strong>Morning Aarti:</strong> {temple.aartiTimings?.morning || "-"}
        </p>
        <p>
          <strong>Shringar Aarti:</strong> {temple.aartiTimings?.shringar || "-"}
        </p>
        <p>
          <strong>Shayan Aarti:</strong> {temple.aartiTimings?.shayan || "-"}
        </p>
        <p>
          <strong>Special Pooja Info:</strong> {display(temple.specialPoojaInfo)}
        </p>
      </div>
      ...
      <div className="border rounded-lg p-4 my-4 bg-white">
        ...
        <p>
          <strong>Dress Code:</strong> {display(temple.dressCode)}
        </p>
        <p>
          <strong>Entry Rules:</strong> {display(temple.entryRules)}
        </p>
        <p>
          <strong>Prohibited Items:</strong>{" "}
          {Array.isArray(temple.prohibitedItems) ? temple.prohibitedItems.join(", ") : temple.prohibitedItems || "-"}
        </p>
        <p>
      ...
      <div className="border rounded-lg p-4 my-4 bg-white">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <p>
            <strong>How to Reach:</strong> {display(temple.howToReach)}
          </p>
          <p>
            <strong>Nearest Airport:</strong> {display(temple.nearestAirport)}
          </p>
          <p>
            <strong>Nearest Railway:</strong> {display(temple.nearestRailway)}
          </p>
          <p>
            <strong>Road Connectivity:</strong> {display(temple.roadConnectivity)}
          </p>
        </div>
      ...
      <div className="border rounded-lg p-4 my-4 bg-white">
        ...
        <p>
          <strong>Created:</strong> {formatDate(temple.createdAt)}
        </p>
        <p>
          <strong>Updated:</strong> {formatDate(temple.updatedAt)}
        </p>
      </div>
    </div>
  );
